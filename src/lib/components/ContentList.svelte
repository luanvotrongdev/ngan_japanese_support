...
<nav class="content-list" aria-label="Session content">
  ...
  <div class="add-row">
    {#each typeOrder as type}
      <a href={`?new=${type}`} class:active={newType === type}>{addLabel[type]}</a>
    {/each}
  </div>
  {#if entries.length}
    <ul>
      {#each entries as entry}{@const key = `${entry.type}:${entry.id}`}
        <li>
          <a class="entry-row" class:selected={key === selectedKey} href={`?entry=${key}`} aria-current={key === selectedKey ? 'true' : undefined}>
